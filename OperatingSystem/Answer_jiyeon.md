...
## 3. 캐시 메모리와 캐시의 지역성(locality)에 대해서 설명해주세요.

### 캐시 메모리
속도가 빠른 장치와 느린 장치간의 속도차에 따른 병목 현상을 줄이기 위한 범용 메모리
주기억장치에서 자주 사용하는 프로그램과 데이터를 저장해두어 처리 속도를 빠르게 함

### 캐시 지역성
캐시메모리의 역할을 제대로 수행하기 위해서는 CPU가 어떤 데이터를 원할 것인가를 어느 정도 예측할 수 있어야 함
캐시의 성능은 작은 용량의 캐시 메모리에 CPU가 이후에 참조할, 쓸모 있는 정보가 어느 정도 들어있느냐에 따라 좌우되기 때문
이 때 적중율(Hit rate)를 극대화 시키기 위해 데이터 지역성(Locality)를 사용
지역성의 전제조건으로 프로그램은 모든 코드나 데이터를 균등하게 Access 하지 않는다는 특성을 기본으로 함. 즉, Locality란 기억 장치 내의 정보를 균일하게가 아닌 어느 한 순간에 특정 부분을 집중적으로 참조하는 특성
...
### 매핑(Mapping) : 캐시 기억장치와 주기억장치 사이에서 정보를 옮기는 것
...
## 4. 커널에 대해서 설명해주세요.
커널은 운영체제의 일부인 시스템 소프트웨어로, 애플리케이션과 하드웨어 간의 인터페이스를 제공

대표적인 커널의 서비스 : 메모리 관리, 프로세스 관리, 자원 접근 및 할당, 파일 시스템 관리

사용자 프로세스가 커널의 서비스를 제공받기 위해서는(커널 영역의 코드를 실행하기 위해서는) 사용자 모드에서 커널 모드로 전환해야 하고, 이는 시스템 호출을 통해 이루어짐

커널의 경우 컴퓨터 자원만 바라보고 있기 때문에 사용자와의 상호작용은 전혀 지원하지 않음
그래서 사용자의 언어를 컴퓨터 명령로 해석하는 시스템 프로그램을 사용

쉘(Shell) : 대표적인 시스템 프로그램으로서 사용자가 컴퓨터에게 전달하는 명령을 해석하는 프로그램. 즉, 커널과 사용자간의 다리 역할을 수행
...
## 8. 스레싱에 대해 설명해주세요.
![image](https://github.com/SsafyStudy13/CS_Study/assets/57094856/fc8156a3-1bcf-446f-99e8-7e7386dde391)

사용하는 프로세스가 많아질 때 어느 한계점까지는 CPU 이용율이 증가하다가 한계점 이상부터는 CPU 이용율이 떨어지게 되는데 이 때 사용량이 떨어지는 이유 중 하나가 스레싱 때문

현재 CPU를 점유하고 있는 프로세스가 필요로 하는 데이터가 물리 메모리에 없다면 swap in과 swap out을 통해 필요한 데이터를 스왑 영역에서 물리 메모리에 올리고 물리 메모리에 있던 어떠한 데이터는 스왑 영역으로 가게 될 것

### 스레싱 : 페이지 부재율이 증가하여 CPU 이용율이 급격하게 떨어지는 현상, 작업이 멈춘 것 같은 상태

페이지 부재 : 프로세스가 필요로 하는 데이터를 요청했을 때 물리 메모리에 위치해있지 않은 경우

발생 이유 : CPU 작업 시간(프로세스 처리 시간)보다 메모리에 적재되지 못한 페이지로 인해 페이지 교체 시간이 증가되어 CPU 이용률 떨어짐,
프로세스에는 너무 적은 프레임을 할당하면 페이지 부재가 자주 일어나게 될 것,
너무 많은 프레임을 할당하면 메모리가 낭비되어 전체적인 시스템에 영향을 끼치게 될 것

### 해결 방법
- 정적 할당 : 프로세스 실행 초기에 프레임을 나눠준 후 크기 고정
    - 균등 할당 : 프로세스 크기에 상관없이 프레임 동일 할당
    - 비례 할당 : 프로세스 크기에 비례하여 프레임 할당
- 동적 할당 : 정적 할당의 사용 도중 메모리 요구 반영 못하는 단점 극복
    - 작업집합 모델 : 지역성 이론(가장 최근에 접근한 프레임이 이후에도 참조될 가능성이 높다.)을 바탕으로 최근 일정 시간 동안 참조된 페이지들을 집합으로 만들고, 물리 메모리에 유지함
        - 작업집합 크기 : 유지할 메모리 크기(작업집합에 들어갈 최대 패이지 수)
        - 작업집합 윈도우 : 작업집합에 포함되는 페이지 범위
- 페이지 부재 빈도 : 프로세스가 필요로 하는 페이지 양을 동적으로 결정
페이지 부재 횟수를 기록하여 프로세스마다 페이지 부재 비율을 계산,
페이지 부재 빈도에서는 페이지 부재 비율의 상한선과 하한선을 설정
    - 페이지 부재 비율이 상한선을 초과하면 할당한 프레임이 적다는 것이므로 프레임 할당량을 늘려줌
    - 페이지 부재 비율이 하한선보다 낮다면 할당한 프레임을 어느정도 회수
